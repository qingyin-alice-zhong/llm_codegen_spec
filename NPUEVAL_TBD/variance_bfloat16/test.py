...
def reference_variance_bfloat16(x0: Annotated[np.ndarray, "shape: (256,)"] -> Annotated[np.ndarray, "shape: (256,)"]):
    return np.array([bfloat16(var)]).astype(np.float32)
# Reference code ends


def _test_variance_bfloat16(kernel_path: str):
    variance_bfloat16_kernel = ExternalModule(
        top="variance_bfloat16",
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="variance_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_variance_bfloat16(args.kernel_path)
