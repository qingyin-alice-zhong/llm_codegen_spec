...
def reference_add_offset_uint8(x: Annotated[np.ndarray, "shape: (1024,)"], offset: Annotated[np.ndarray, "shape: (1,)"]) -> Annotated[np.ndarray, "shape: (1024,)"]:
    return (x + int(offset[0])).astype(np.uint8)
# Reference code ends


def _test_add_offset_uint8(kernel_path: str):
    ...
    input_tensor = np.random.randint(0, 250, (1024,), dtype=np.uint8)  # Leave room for offset
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="add_offset_uint8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_add_offset_uint8(args.kernel_path)
