...
def reference_floor_bfloat16(x0: Annotated[np.ndarray, "shape: (256,)"] -> Annotated[np.ndarray, "shape: (256,)"]):
    return np.floor(x0).astype(bfloat16)
# Reference code ends


def _test_floor_bfloat16(kernel_path: str):
    floor_bfloat16_kernel = ExternalModule(
        top="floor_bfloat16",
    ...
    def top():
        ...
        def core(A: bfloat16[M] @ Ly, B: bfloat16[M] @ Ly):
            floor_bfloat16_kernel(A, B)



    ref_output = reference_floor_bfloat16()
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="floor_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_floor_bfloat16(args.kernel_path)
