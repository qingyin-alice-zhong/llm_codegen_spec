...
def reference_tan_bfloat16(x0: Annotated[np.ndarray, "shape: (3,)"] -> Annotated[np.ndarray, "shape: (3,)"]):
    return np.tan(x).astype(bfloat16)
# Reference code ends


def _test_tan_bfloat16(kernel_path: str):
    tan_bfloat16_kernel = ExternalModule(
        top="tan_bfloat16",
    ...
    def top():
        @df.kernel(mapping=[1])
        def core(A: bfloat16[M] @ Ly, B: bfloat16[3] @ Ly):
            tan_bfloat16_kernel(A, B)

    input_tensor1 = np.random.randint(-100, 100, (3,), dtype=np.int8)

    ref_output = reference_tan_bfloat16(input_tensor1)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="tan_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_tan_bfloat16(args.kernel_path)
