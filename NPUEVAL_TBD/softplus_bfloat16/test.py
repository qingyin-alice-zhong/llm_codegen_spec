...
def reference_softplus_bfloat16(x: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    # Softplus(x) = ln(1 + e^x)
    return np.log(1 + np.exp(np.clip(x, -500, 500))).astype(np.float32)
# Reference code ends


def _test_softplus_bfloat16(kernel_path: str):
    softplus_kernel = ExternalModule(
        top="softplus_bfloat16",
    ...
    input_tensor = np.random.randn(tensor_size).astype(np.float32) * 2  # Scale to reasonable range
    ref_output = reference_softplus_bfloat16(input_tensor)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="softplus_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_softplus_bfloat16(args.kernel_path)
