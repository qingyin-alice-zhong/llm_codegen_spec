...
def reference_bitwisexor_uint8(x0: Annotated[np.ndarray, "shape: (1024,)"], x1: Annotated[np.ndarray, "shape: (1024,)"] -> Annotated[np.ndarray, "shape: (1024,)"]):
    return np.bitwise_xor(x0, x1).astype(np.uint8)
# Reference code ends


def _test_bitwisexor_uint8(kernel_path: str):
    bitwisexor_uint8_kernel = ExternalModule(
        top="bitwisexor_uint8",
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="bitwisexor_uint8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_bitwisexor_uint8(args.kernel_path)
