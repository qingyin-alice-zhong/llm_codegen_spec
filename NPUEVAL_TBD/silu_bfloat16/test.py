...
def reference_silu_bfloat16(x0: Annotated[np.ndarray, "shape: (256,)"] -> Annotated[np.ndarray, "shape: (256,)"]):
    return (x / (1 + np.exp(-x))).astype(bfloat16)
# Reference code ends


def _test_silu_bfloat16(kernel_path: str):
    silu_bfloat16_kernel = ExternalModule(
        top="silu_bfloat16",
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="silu_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_silu_bfloat16(args.kernel_path)
