...
def reference_softmax_bfloat16(x0: Annotated[np.ndarray, "shape: (64,)"] -> Annotated[np.ndarray, "shape: (64,)"]):
    return (exp_x / np.sum(exp_x)).astype(bfloat16)
# Reference code ends


def _test_softmax_bfloat16(kernel_path: str):
    softmax_bfloat16_kernel = ExternalModule(
    ...
    def top():
        ...
        def core(A: bfloat16[M] @ Ly, B: bfloat16[64] @ Ly):
            softmax_bfloat16_kernel(A, B)

    input_tensor1 = np.random.randint(-100, 100, (64,), dtype=np.int8)
    input_tensor2 = np.random.randint(-100, 100, (64,), dtype=np.int8)

    ref_output = reference_softmax_bfloat16(input_tensor1, input_tensor2)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        output_allo = np.zeros((64,), dtype=np.float32)
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="softmax_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_softmax_bfloat16(args.kernel_path)
