...
def reference_bitwisenot_uint8(x0: Annotated[np.ndarray, "shape: (1024,)"] -> Annotated[np.ndarray, "shape: (1024,)"]):
    return np.bitwise_not(x0).astype(np.uint8)
# Reference code ends


def _test_bitwisenot_uint8(kernel_path: str):
    bitwisenot_uint8_kernel = ExternalModule(
        top="bitwisenot_uint8",
    ...
    def top():
        @df.kernel(mapping=[1])
        def core(A: uint8[M] @ Ly, B: uint8[M] @ Ly):
            bitwisenot_uint8_kernel(A, B)

    input_tensor1 = np.random.randint(-100, 100, (1024,), dtype=np.uint8)

    ref_output = reference_bitwisenot_uint8(input_tensor1)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="bitwisenot_uint8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_bitwisenot_uint8(args.kernel_path)
