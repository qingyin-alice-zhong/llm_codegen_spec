...
def reference_max_abs_bfloat16(x0: Annotated[np.ndarray, "shape: (256,)"] -> Annotated[np.ndarray, "shape: (256,)"]):
    return np.array([bfloat16(absmax)]).astype(np.float32)
# Reference code ends


def _test_max_abs_bfloat16(kernel_path: str):
    max_abs_bfloat16_kernel = ExternalModule(
        top="max_abs_bfloat16",
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="max_abs_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_max_abs_bfloat16(args.kernel_path)
