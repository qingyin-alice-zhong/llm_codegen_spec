...
def reference_leaky_relu_bfloat16(x0: Annotated[np.ndarray, "shape: (256,)"] -> Annotated[np.ndarray, "shape: (256,)"]):
    return np.where(x0 > 0, x0, x0 * bfloat16(0.01)).astype(np.float32)
# Reference code ends


def _test_leaky_relu_bfloat16(kernel_path: str):
    leakyrelu_bfloat16_kernel = ExternalModule(
        top="leaky_relu_bfloat16",
    ...
    def top():
        ...
        def core(A: bfloat16[M] @ Ly, B: bfloat16[M] @ Ly):
            leakyrelu_bfloat16_kernel(A, B)



    ref_output = reference_leaky_relu_bfloat16()
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="leaky_relu_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_leaky_relu_bfloat16(args.kernel_path)
