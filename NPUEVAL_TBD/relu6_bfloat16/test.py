...
def reference_relu6_bfloat16(x0: Annotated[np.ndarray, "shape: (256,)"] -> Annotated[np.ndarray, "shape: (256,)"]):
    return np.minimum(np.maximum(x0.astype(np.float32), 0.0), 6.0).astype(bfloat16)
# Reference code ends


def _test_relu6_bfloat16(kernel_path: str):
    relu6_bfloat16_kernel = ExternalModule(
        top="relu6_bfloat16",
    ...
    def top():
        ...
        def core(A: bfloat16[M] @ Ly, B: bfloat16[M] @ Ly):
            relu6_bfloat16_kernel(A, B)



    ref_output = reference_relu6_bfloat16()
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="relu6_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_relu6_bfloat16(args.kernel_path)
