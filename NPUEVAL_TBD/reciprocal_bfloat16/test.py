...
def reference_reciprocal_bfloat16(x0: Annotated[np.ndarray, "shape: (256,)"] -> Annotated[np.ndarray, "shape: (256,)"]):
    return result.astype(bfloat16)
# Reference code ends


def _test_reciprocal_bfloat16(kernel_path: str):
    reciprocal_bfloat16_kernel = ExternalModule(
        top="reciprocal_bfloat16",
    ...
    def top():
        ...
        def core(A: bfloat16[M] @ Ly, B: bfloat16[256] @ Ly):
            reciprocal_bfloat16_kernel(A, B)

    input_tensor1 = np.random.randn(256).astype(np.float32)

    ref_output = reference_reciprocal_bfloat16(input_tensor1)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="reciprocal_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_reciprocal_bfloat16(args.kernel_path)
