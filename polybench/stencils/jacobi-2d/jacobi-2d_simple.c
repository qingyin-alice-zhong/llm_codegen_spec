...
// 2D Jacobi stencil computation
// Performs iterative smoothing on a 2D grid
void kernel_jacobi_2d(int tsteps, int n, double** A, double** B)
{
    int t, i, j;

    for (t = 0; t < tsteps; t++) {
        // Update B based on A using 5-point stencil
        for (i = 1; i < n - 1; i++) {
            for (j = 1; j < n - 1; j++) {
        ...
        // Update A based on B using 5-point stencil
        for (i = 1; i < n - 1; i++) {
            for (j = 1; j < n - 1; j++) {
...
void kernel_jacobi_2d_optimized(int tsteps, int n, double** A, double** B)
{
...
{
    printf("Verifying 2D stencil results with tolerance %e...\n", tolerance);
...
int main(int argc, char** argv)
{
    ...
    double** B_orig = allocate_2d_array(n, n);
    ...
    copy_matrix(n, n, B_orig, B1);
    copy_matrix(n, n, A_orig, A2);
    copy_matrix(n, n, B_orig, B2);

    // Run performance tests
    printf("Running performance tests (%d runs each)...\n", num_runs);
    
    double original_time = run_performance_test(kernel_jacobi_2d, tsteps, n,
                                              A_orig, B_orig, A1, B1, num_runs);
    
    double optimized_time = run_performance_test(kernel_jacobi_2d_optimized, tsteps, n,
