...
def reference_negate_int8(x: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    return (-x).astype(np.int8)
# Reference code ends


def _test_negate_int8(kernel_path: str):
    negate_kernel = ExternalModule(
        top="negate_int8",
    ...
    input_tensor = np.random.randint(-128, 128, size=tensor_size, dtype=np.int8)
    ref_output = reference_negate_int8(input_tensor)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="negate_int8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_negate_int8(args.kernel_path)
