...
def reference_cast_bfloat16_to_float32(x: Annotated[np.ndarray, "shape: (512,)"]) -> Annotated[np.ndarray, "shape: (512,)"]:
    return x.astype(np.float32)
# Reference code ends


def _test_cast_bfloat16_to_float32(kernel_path: str):
    cast_bfloat16_to_float32_kernel = ExternalModule(
        top="cast_bfloat16_to_float32",
    ...
    def top():
        @df.kernel(mapping=[1])
        def core(A: bfloat16[M] @ Ly, B: float32[M] @ Ly):
            cast_bfloat16_to_float32_kernel(A, B)

    input_tensor = np.random.randn(tensor_size).astype(np_bfloat16)
    ref_output = reference_cast_bfloat16_to_float32(input_tensor)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="cast_bfloat16_to_float32", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_cast_bfloat16_to_float32(args.kernel_path)
