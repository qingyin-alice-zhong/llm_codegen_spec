...
def reference_hardsigmoid_int8(x: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    x_f = x.astype(np.float32)
    y = x_f / 6.0 + 0.5
    y = np.clip(y, 0.0, 1.0)
    out = (y * 127.0 + 0.5).astype(np.int32)
    out = np.clip(out, 0, 127)
    return out.astype(np.int8)
# Reference code ends


def _test_hardsigmoid_int8(kernel_path: str):
    hardsigmoid_int8_kernel = ExternalModule(
        top="hardsigmoid_int8",
    ...
    def top():
        ...
        def core(A: int8[M] @ Ly, B: int8[M] @ Ly):
            hardsigmoid_int8_kernel(A, B)

    input_tensor = np.random.randint(-100, 100, (tensor_size,), dtype=np.int8)
    ref_output = reference_hardsigmoid_int8(input_tensor)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="hardsigmoid_int8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_hardsigmoid_int8(args.kernel_path)
