...
def reference_conv1d_bias_relu_bfloat16(in_buffer: Annotated[np.ndarray, "shape: (256,)"], param: Annotated[np.ndarray, "shape: (6,)"]) -> Annotated[np.ndarray, "shape: (252,)"]:
    # Extract kernel[4], bias, stride from param (6 float32 values packed as int32)
    ...
    kernel_size = 4
    output_size = min((vector_size - kernel_size) // stride + 1, 252)  # Limit to 252
    out_buffer = np.zeros(output_size, dtype=np_bfloat16)
    for i in range(output_size):
        acc = 0.0
        for j in range(kernel_size):
            prod = np_bfloat16(float(in_buffer[i * stride + j]) * float(kernel[j]))
            acc = np_bfloat16(float(acc) + float(prod))
        acc = np_bfloat16(float(acc) + float(bias))
        if float(acc) < 0.0:
            acc = np_bfloat16(0.0)
        out_buffer[i] = acc
    return out_buffer
...
def _test_conv1d_bias_relu_bfloat16(kernel_path: str):
    conv1d_bias_relu_bfloat16_kernel = ExternalModule(
        top="conv1d_bias_relu_bfloat16",
    ...
    def top():
        @df.kernel(mapping=[1])
        def core(A: bfloat16[M] @ Ly, C: bfloat16[252] @ Ly, Param: int32[6] @ Ly):
            conv1d_bias_relu_bfloat16_kernel(A, C, Param)

    # Generate random bfloat16 input and parameters
    in_buffer = (np.random.randn(tensor_size) * 3.2).astype(np_bfloat16)
    
    # Pack kernel[4], bias, stride into param array (6 float32 values as int32)
    kernel_vals = (np.random.randn(4) * 0.5).astype(np.float32)
    bias_val = np.random.randn() * 0.1
    stride_val = 1  # Use stride=1 for output size calculation
    
    param = np.array([
        kernel_vals[0].view(np.int32), kernel_vals[1].view(np.int32),
        kernel_vals[2].view(np.int32), kernel_vals[3].view(np.int32),
        np.float32(bias_val).view(np.int32), stride_val
    ], dtype=np.int32)
    
    ref_output = reference_conv1d_bias_relu_bfloat16(in_buffer, param.astype(np.float32))
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="conv1d_bias_relu_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_conv1d_bias_relu_bfloat16(args.kernel_path)
