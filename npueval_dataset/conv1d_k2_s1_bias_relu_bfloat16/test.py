...
def reference_conv1d_k2_s1_bias_relu_bfloat16(in_buffer: Annotated[np.ndarray, "shape: (256,)"], param: Annotated[np.ndarray, "shape: (3,)"]) -> Annotated[np.ndarray, "shape: (254,)"]:
    # Extract kernel[2] and bias from param (3 float32 values packed as int32)
    ...
    bias_val = np.array([param[2]], dtype=np.int32).view(np.float32)[0]
    ...
    kernel_size = 2
    stride = 1
    output_size = min((vector_size - kernel_size) // stride + 1, 254)  # Limit to 254 for 4-byte alignment
...
def _test_conv1d_k2_s1_bias_relu_bfloat16(kernel_path: str):
    conv1d_k2_s1_bias_relu_bfloat16_kernel = ExternalModule(
        top="conv1d_k2_s1_bias_relu_bfloat16",
    ...
    def top():
        ...
        def core(A: bfloat16[M] @ Ly, C: bfloat16[254] @ Ly, Param: int32[3] @ Ly):
            conv1d_k2_s1_bias_relu_bfloat16_kernel(A, C, Param)

    # Generate random bfloat16 input and parameters
    in_buffer = (np.random.randn(256) * 3.2).astype(np_bfloat16)
    
    # Pack kernel[2] and bias into param array (3 float32 values as int32)
    ...
    ref_output = reference_conv1d_k2_s1_bias_relu_bfloat16(in_buffer, param.astype(np.float32))
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="conv1d_k2_s1_bias_relu_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_conv1d_k2_s1_bias_relu_bfloat16(args.kernel_path)
