...
void conv1d_k2_s1_bias_relu_bfloat16(bfloat16 in_buffer[256], bfloat16 out_buffer[254], std::int32_t param[3]) {
    event0();
    
    // Unpack parameters: kernel[2], bias (all stored as float32 in int32)
    bfloat16 kernel[2];
    kernel[0] = (bfloat16)(*(float*)&param[0]);
    kernel[1] = (bfloat16)(*(float*)&param[1]);
    bfloat16 bias = (bfloat16)(*(float*)&param[2]);
    
    constexpr std::int32_t VECTOR_SIZE = 256;
    constexpr std::int32_t KERNEL_SIZE = 2;
    constexpr std::int32_t STRIDE = 1;
    std::int32_t num_windows = 254;  // Fixed output size for 4-byte alignment
    ...
    for (std::int32_t i = 0; i < num_windows; i++) {
        ...
        acc += (float)bias;
        float relu = acc > 0.0f ? acc : 0.0f;  // ReLU
