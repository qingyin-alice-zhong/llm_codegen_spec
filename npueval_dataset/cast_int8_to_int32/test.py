...
def reference_cast_int8_to_int32(x: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    return x.astype(np.int32)
# Reference code ends


def _test_cast_int8_to_int32(kernel_path: str):
    cast_int8_to_int32_kernel = ExternalModule(
        top="cast_int8_to_int32",
    ...
    def top():
        @df.kernel(mapping=[1])
        def core(A: int8[M] @ Ly, B: int32[M] @ Ly):
            cast_int8_to_int32_kernel(A, B)

    input_tensor = np.random.randint(-100, 100, (tensor_size,), dtype=np.int8)
    ref_output = reference_cast_int8_to_int32(input_tensor)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="cast_int8_to_int32", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_cast_int8_to_int32(args.kernel_path)
