...
def reference_vectorsubtract_bfloat16(x0: Annotated[np.ndarray, "shape: (256,)"], x1: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    return (x0 - x1).astype(np_bfloat16)
# Reference code ends


def _test_vectorsubtract_bfloat16(kernel_path: str):
    vectorsubtract_bfloat16_kernel = ExternalModule(
        top="vectorsubtract_bfloat16",
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="vectorsubtract_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_vectorsubtract_bfloat16(args.kernel_path)
