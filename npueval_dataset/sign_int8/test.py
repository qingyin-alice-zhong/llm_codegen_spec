...
def reference_sign_int8(x: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    return np.sign(x).astype(np.int8)
# Reference code ends


def _test_sign_int8(kernel_path: str):
    sign_int8_kernel = ExternalModule(
        top="sign_int8",
    ...
    def top():
        @df.kernel(mapping=[1])
        def core(A: int8[M] @ Ly, B: int8[tensor_size] @ Ly):
            sign_int8_kernel(A, B)

    input_tensor1 = np.random.randint(-100, 100, (tensor_size,), dtype=np.int8)

    ref_output = reference_sign_int8(input_tensor1)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="sign_int8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_sign_int8(args.kernel_path)
