...
def reference_avgpool2d_relu_bfloat16(input_flat: Annotated[np.ndarray, "shape: (1024,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    # Reshape flat input to 32x32 matrix
    input_2d = input_flat.reshape((32, 32))
    rows, cols = input_2d.shape
    out_rows = rows // 2
    out_cols = cols // 2
    output = np.zeros((out_rows, out_cols), dtype=input_2d.dtype)
    for i in range(0, rows, 2):
        for j in range(0, cols, 2):
            window = input_2d[i:i+2, j:j+2].astype(np.float32)
            avg = window.mean()
            relu = max(avg, 0.0)  # ReLU activation
            output[i//2, j//2] = np_bfloat16(relu)
    return output.flatten().astype(np_bfloat16)
# Reference code ends


def _test_avgpool2d_relu_bfloat16(kernel_path: str):
    avgpool2d_relu_bfloat16_kernel = ExternalModule(
        top="avgpool2d_relu_bfloat16",
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="avgpool2d_relu_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_avgpool2d_relu_bfloat16(args.kernel_path)
