...
def reference_elementwise_min_int8(x0: Annotated[np.ndarray, "shape: (256,)"], x1: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    return np.minimum(x0, x1).astype(np.int8)
# Reference code ends


def _test_elementwise_min_int8(kernel_path: str):
    elementwise_min_int8_kernel = ExternalModule(
        top="elementwise_min_int8",
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="elementwise_min_int8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_elementwise_min_int8(args.kernel_path)
