...
def reference_ceil_bfloat16(x: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    return np.ceil(x).astype(np_bfloat16)
# Reference code ends


def _test_ceil_bfloat16(kernel_path: str):
    ceil_bfloat16_kernel = ExternalModule(
        top="ceil_bfloat16",
    ...
    def top():
        ...
        def core(A: bfloat16[M] @ Ly, B: bfloat16[M] @ Ly):
            ceil_bfloat16_kernel(A, B)

    input_tensor = np.random.randn(tensor_size).astype(np_bfloat16)
    ref_output = reference_ceil_bfloat16(input_tensor)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="ceil_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_ceil_bfloat16(args.kernel_path)
