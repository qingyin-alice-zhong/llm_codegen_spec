...
def reference_compare_equal_int32(x0: Annotated[np.ndarray, "shape: (256,)"], x1: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (256,)"]:
    return (x0 == x1).astype(np.int32)
# Reference code ends


def _test_compare_equal_int32(kernel_path: str):
    compare_equal_int32_kernel = ExternalModule(
        top="compare_equal_int32",
    ...
    def top():
        @df.kernel(mapping=[1])
        def core(A: int32[M] @ Ly, B: int32[M] @ Ly, C: int32[M] @ Ly):
            compare_equal_int32_kernel(A, B, C)
    ...
    ref_output = reference_compare_equal_int32(input_tensor1, input_tensor2)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        output_allo = np.zeros((tensor_size,), dtype=np.int32)
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="compare_equal_int32", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_compare_equal_int32(args.kernel_path)
