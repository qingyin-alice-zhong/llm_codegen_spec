...
def reference_reduce_add_relu_int8(x: Annotated[np.ndarray, "shape: (128,)"]) -> Annotated[np.ndarray, "shape: (1,)"]:
    s = np.sum(x.astype(np.int32))
    s = max(s, 0)
    s = np.clip(s, -128, 127)
...
def _test_reduce_add_relu_int8(kernel_path: str):
    reduce_add_relu_int8_kernel = ExternalModule(
        top="reduce_add_relu_int8",
    ...
    def top():
        @df.kernel(mapping=[1])
        def core(A: int8[M] @ Ly, B: int32[1] @ Ly):
            reduce_add_relu_int8_kernel(A, B)

    # fix random seed
    input_tensor1 = np.random.randint(-100, 100, (40,), dtype=np.int8)

    ref_output = reference_reduce_add_relu_int8(input_tensor1)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="reduce_add_relu_int8", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_reduce_add_relu_int8(args.kernel_path)
