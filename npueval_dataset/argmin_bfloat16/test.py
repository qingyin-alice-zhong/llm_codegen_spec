...
def reference_argmin_bfloat16(x: Annotated[np.ndarray, "shape: (256,)"]) -> Annotated[np.ndarray, "shape: (1,)"]:
    return np.array([np.argmin(x)], dtype=np.int32)
# Reference code ends


def _test_argmin_bfloat16(kernel_path: str):
    argmin_bfloat16_kernel = ExternalModule(
        top="argmin_bfloat16",
    ...
    def top():
        ...
        def core(A: bfloat16[M] @ Ly, B: int32[1] @ Ly):
            argmin_bfloat16_kernel(A, B)

    input_tensor = np.random.randn(tensor_size).astype(np_bfloat16)
    ref_output = reference_argmin_bfloat16(input_tensor)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        # ===== Analyze trace via shared utility if generated under top.prj/ =====
        analyze_trace(top_prj_dir=TOP_PRJ_ABS_DIR, targetname="argmin_bfloat16", colshift=1)
...
if __name__ == "__main__":
    ...
    _test_argmin_bfloat16(args.kernel_path)
