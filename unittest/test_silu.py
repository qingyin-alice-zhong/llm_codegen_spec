...
def silu(x: Annotated[torch.Tensor, "shape: (1024,)"]) -> Annotated[torch.Tensor, "shape: (1024,)"]:
    """
...
def _test_silu(kernel_path: str):
    
    silu_kernel = ExternalModule(
        top="silu_int16_vector",
    ...
    output_float = silu(input_float)
    output = output_float.to(torch.int16)
    ...
    if "MLIR_AIE_INSTALL_DIR" in os.environ:
        ...
        try:
            # Use a higher tolerance due to quantization effects
            np.testing.assert_allclose(output_allo, output, rtol=1e-1, atol=1)
            print("PASS!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kernel_path", type=str, default="kernels_func/silu.cc")
    args = parser.parse_args()

    _test_silu(args.kernel_path)